...
% \VignetteIndexEntry{Introduction to turnlist}
% \VignetteEngine{knitr}

\begin{document}
\title{Introduction to \texttt{turnlist}}
\author{
 Gaston Sanchez
}
\date{}
...
\texttt{turnlist} is an R package designed to provide a set of handy functions for manipulating lists of vectors. The main idea is to make it easier to turn lists into other data R structures. \texttt{turnlist} was born out of necessity from my involvement with \textbf{Multiblock Methods} and other multivariate data analysis methods (eg PLS Path Modeling). 
...
To see how we can apply \texttt{turnlist}, we need to consider some data under a multiblock perspective. First let's start by creating a data matrix with 10 observations and 9 variables.
<<data_matrix>>=
# create a matrix
set.seed = 21
...
\texttt{turnlist} has been designed to work with lists (preferable of vectors) in order to turn them into \textit{indexed structures}. Such structures are mainly vectors that map the position indices of the elements in the list.
